...
function actualizarBotonAudio() {
    if (state.isAudioActive) {
        DOM.audioStatusText.textContent = "Sonido Activo";
        ...
        DOM.audioStatusText.textContent = "Silenciado";
...
function enforcePhysiologicConstraints() {
    ...
    if (state.paDia < 40) state.paDia = 40;
    if (state.paSys > 220) state.paSys = 220;
}
...
function dibujarTrazoSweep(ctx, config, width, height, newY) {
    ...
    // 2. Dibujar línea (En blanco si las respuestas están ocultas)
    const isHidden = document.body.classList.contains('hide-answers');
    const drawColor = isHidden ? '#ffffff' : config.color;
...
function actualizarEstadoGeneral() {
    ...
    if (isUnstable) {
        DOM.patientStatus.textContent = "PACIENTE INESTABLE";
